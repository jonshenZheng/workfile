...
let startY,                 //触摸开始y坐标
    cvPercent = 0.5,         //超过这个节点的比例就覆盖
    cvlimit = [],            //超过这个高度就直接遮罩分类层
    cvHei = [],              //要被覆盖的节点高度
...
Page({
  ...
  touchstartFn : function(e){
    ...
    console.log('cvHei'+cvHei[ind] + ' startY:' + startY);
  },
  touchmoveFn : function(e){

    let ind = this.data.curIndex,
        arr = this.data.touchCvData; 
    ...
    arr[ind].scaleY = cy;
    arr[ind].height = t;
    arr[ind].animateName = cname;

    //console.log('pointY:' + pointY + ' startY:' + startY + ' cvHei:' + cvHei[ind]+' height:' + t );
  ...
  touchendFn : function(){
      ...
      if (!touchLock[ind]) {
          ...
          arr[ind].animateName = animateNameKey;
          arr[ind].scaleY = 1;
          arr[ind].height = cvHei[ind];
  ...
  initTouchData : function(){
      ...
      if (!cvHei.length) {
          for (; cg_i < cgLen; cg_i++) {
              cvlimit[cg_i] = 'none';
              cvHei[cg_i] = 'none';
              touchLock[cg_i] = false;
              obj[cg_i] = {};
              obj[cg_i].height = 'auto';
              obj[cg_i].scaleY = 1;
              obj[cg_i].animateName = '';
          }
  ...
  getCVHeiFn : function(ind){
      ...
      if (cvHei[ind] === 'none'){
          wx.createSelectorQuery().select('#selectArea-id' + ind).boundingClientRect(function (rect) {
              cvHei[ind] = rect.height;
              cvlimit[ind] = rect.height * cvPercent;
          }).exec();
      }
  }
